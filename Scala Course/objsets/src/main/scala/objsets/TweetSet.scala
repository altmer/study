...
abstract class TweetSet {
  ...
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = 
    if (! this.isEmpty) this.tail.filter0(p, if (p(this.head)) accu.incl(this.head) else accu ) 
    else accu

  def union(that: TweetSet): TweetSet = 
    if ( ! that.isEmpty) this.incl(that.head).union(that.tail)
    else this

  // Hint: the method "remove" on TweetSet will be very useful.
  def ascendingByRetweet: Trending = {
      def ascendingByRetweetRec(curr: Trending, set: TweetSet): Trending = 
        if (set.isEmpty) curr
        else ascendingByRetweetRec(curr + set.findMin, set.remove(set.findMin))
      ascendingByRetweetRec(new EmptyTrending, this)
  }
...
object GoogleVsApple {
  ...
  val googleTweets: TweetSet = TweetReader.allTweets.filter(tw => google.exists( keyword => tw.text.contains(keyword) )   )

  val appleTweets: TweetSet = TweetReader.allTweets.filter(tw => apple.exists( keyword => tw.text.contains(keyword) )   )
