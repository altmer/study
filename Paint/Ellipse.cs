...
namespace Kamikaze
{
	...
	{
		public Ellipse(Point p1, Point p2, Color colline, Color colback, int width, bool fill) : base(p1, p2, colline, colback, width, fill) { }
		...
		public override void Draw(Graphics g, int shiftX, int shiftY) {
			...
			if(fill)
				g.FillEllipse(new SolidBrush(colorBackground), Rectangle.FromLTRB(x1, y1, x2, y2));
			...
			g.DrawEllipse(new Pen(color, width), Rectangle.FromLTRB(x1, y1, x2, y2));
		...
		public override void DrawDash(Graphics g, int shiftX, int shiftY) {
			...
			pen.DashStyle = System.Drawing.Drawing2D.DashStyle.Dash;
			g.DrawEllipse(pen, Rectangle.FromLTRB(x1, y1, x2, y2));
		}
		...
		public override object Clone(){
			return new Ellipse ( startPoint, endPoint, color, colorBackground, width, fill );
		}
